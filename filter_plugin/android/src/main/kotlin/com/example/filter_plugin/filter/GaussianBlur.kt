package com.example.filter_plugin.filter
...
class GaussianBlur(private val outSurface: Surface, private val srcImg: Bitmap) {
   ...
   private val fragmentShader = """#version 300 es
    ...
    private fun eglSetup() {
        ...
        if (mEGLDisplay == EGL14.EGL_NO_DISPLAY) throw RuntimeException("Unable to get EGL 14 display")
        ...
        if (numConfig[0] == 0) throw Exception("No EGL config was available")
        ...
        mEGLSurface =
            EGL14.eglCreateWindowSurface(mEGLDisplay, configs[0], outSurface, surfaceAttribs, 0)
    ...
    private fun programSetup() {
        // Create the program
        this.program = GLUtils.createProgram(GLUtils.vertexShaderSource, fragmentShader)
        ...
        // Provide texture coordinates to the vertex shader, we use 2 rectangle that will cover the
        // entire image
        ...
        // Upload the texCoords into the buffer
        GLES30.glBufferData(
            GLES30.GL_ARRAY_BUFFER,
            texCoordsBuffer.capacity() * 4,
            texCoordsBuffer,
            GLES30.GL_STATIC_DRAW
        )
        // Turn it "on"
        GLES30.glEnableVertexAttribArray(this.attributes["a_texCoord"]!!)
        // Describe how to pull data out of the buffer, take two items per iteration (x and y)
        GLES30.glVertexAttribPointer(
            this.attributes["a_texCoord"]!!, 2, GLES30.GL_FLOAT, false, 0, 0
        )
    }
    ...
    fun draw(radius: Float, flip: Boolean = false) {
        ...
        GLES30.glUniform1f(
            this.uniforms["u_flipY"]!!, if (flip) -1f else 1f
        ...
        // Unbind any output frame buffer that may have bounded by other OpenGL programs (so we render to the default display)
